...
import com.lmax.ticketing.web.json.*;
...
public class ResponseServlet extends HttpServlet implements EventHandler<Message> {
    ...
    private final PriceUpdatedToJson priceUpdatedToJson = new PriceUpdatedToJson();
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
            ...
            useUDP = Boolean.parseBoolean(config.getInitParameter("useUDP"));

            Disruptor<Message> disruptor = new Disruptor<Message>(Message.FACTORY, 1024, executor);
            disruptor.handleEventsWith(this);
            ringBuffer = disruptor.start();

            LOGGER.info("Response Servlet Using " + (useUDP ? "UDP" : "RabbitMQ"));
            if (useUDP) {
    ...
    public void onEvent(Message message, long sequence, boolean endOfBatch) throws Exception {
        ...
        switch (type) {
            ...
            case PRICE_UPDATED:
                final PriceUpdated priceUpdated = message.event.asPriceUpdated;
                JSONObject priceUpdatedAsJson = priceUpdatedToJson.toJson(priceUpdated);
                enqueueEvent(priceUpdatedAsJson);
